...
public class HttpDataLoaderTest {
    ...
    public void parseJsonData() {
        ...
        String fileContent="{\n" +
                ...
                "  ]\n" +
                "}";
        List<ShopLocation> location=httpDataLoader.ParseJsonData(fileContent);

        Assert.assertEquals(3,location.size());
        Assert.assertEquals("暨大珠海",location.get(0).getName());
        Assert.assertEquals(22.251953,location.get(2).getLatitude(),1e-6);
        Assert.assertEquals(113.526421,location.get(2).getLongitude(),1e-6);
        Assert.assertEquals("珠海二城广场",location.get(2).getMemo());
